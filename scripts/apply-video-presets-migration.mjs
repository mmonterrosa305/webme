...
async function runViaDeployedApi() {
  ...
  const response = await fetch(
    `${appUrl.replace(/\/$/, "")}/api/admin/migrate-video-presets`,
    {
      method: "POST",
      headers: {
...
async function verifyMigration() {
  ...
  if (error?.message.includes("does not exist")) {
    throw new Error("Verification failed: video_presets table does not exist.");
  }

  console.log("Verified video_presets table exists.", {
